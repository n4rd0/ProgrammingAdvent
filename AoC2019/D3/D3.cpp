	...
	for (int i = 0; i < wire1.size(); i++) {
		...
		for (int j = 0; j < wire2.size(); j++) {
			...
			if (x == search.x && y == search.y) {
				...
				if (min_step > (search.step + wire2[j].step)) min_step = search.step + wire2[j].step;

				cout << search.x << " " << search.y << " " << search.step+wire2[j].step << endl;
			}
		}
	}

	cout << "Answer 2 = " << min_step << endl;
	return repeated;
}
point D31_3(vector<point> repeated) {
	...
	for (int i = 1; i < repeated.size(); i++) {
		if ((abs(res.x) + abs(res.y)) > (abs(repeated[i].x) + abs(repeated[i].y))) res = repeated[i]; 
	}
